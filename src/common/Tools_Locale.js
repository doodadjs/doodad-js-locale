...
(function() {
	...
	global.DD_MODULES['Doodad.Tools.Locale'] = {
		type: null,
		version: '0b',
		namespaces: null,
...
*/
			...
			__Internal__.getFileUrl = function(fileName) {
				var localesPath = tools.options.settings.localesPath;
				if (!localesPath) {
					localesPath = './res/locales/';
				};

				localesPath = tools.options.hooks.pathParser(localesPath);

				localesPath = tools.getCurrentScript((global.document?document.currentScript:module.filename)||(function(){try{throw new Error("");}catch(ex){return ex;}})()).combine(localesPath, {
					file: null,
				});
				
				var path = tools.options.hooks.pathParser(fileName);

				return localesPath.combine(path);
			};
...
/* NOT NEEDED FOR THE MOMENT
			__Internal__.loadCategories = function loadCategories() {
				return files.readFile(__Internal__.getFileUrl('categories.def'), {async: true, encoding: 'utf8', enableCache: true})['finally'](function proceed(data) {
					var __items = {};
...
*/
			...
			__Internal__.loadLocale = function loadLocaleInternal(name, /*optional*/category, /*optional*/sections) {
				...
				return files.readFile(__Internal__.getFileUrl(name), {async: true, encoding: 'utf8', enableCache: true})
					['finally'](function proceed(data) {
						return __Internal__.parseLocaleFile(data, __Internal__.categories, category, sections)
							['finally'](function(loc) {
								loc = {
									categories: loc,
								};
			...
			locale.setLocale = function setLocale(/*optional*/category, /*optional*/name) {
				...
				return locale.loadLocale(name)['finally'](function(loc) {
			...
			return function init(/*optional*/options) {
				// NOT NEEDED FOR THE MOMENT
				//__Internal__.loadCategories();
					var readyCb = new namespaces.ReadyCallback();
					locale.setLocale( locale.LC_ALL, "")['finally'](readyCb);
				//locale.loadLocale('fr_CA')['finally'](function(locale) {
				//	console.log(locale);
				//});
			};
		},
